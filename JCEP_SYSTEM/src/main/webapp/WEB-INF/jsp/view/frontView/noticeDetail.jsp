...
<script type="text/javaScript" >
	...
	function moveDetailViewPaging(noticeIdx, pageNum) {
					...
					<div class="daylist">
						<table class="table01">
							...
							<tr>
								<th>내용</th>
								<td colspan="4">
									<c:out escapeXml="false" value="${detail.noticeCont}"/>
								</td>								
							</tr>
							<tr>
								<th>첨부파일</th>
								<td colspan="4">
								<a href="/db/fileDownload.do?fileNum=${detail.noticeIdx}">
						...
						<ul class="listpn">
							<li>
								<strong>이전글</strong>
									<c:if test="${detail2.noticeIdx ==null}">	// 
											<a>이전글이 없습니다.</a>
									</c:if>
									<c:if test="${detail2.noticeIdx != null}">
											<a href="javascript:moveDetailViewPaging('${detail2.noticeIdx}','${pageNum-1}');" id="snominus"><c:out value="${detail2.noticeTitle}"/></a>
									</c:if>
							</li>
							<li>
								<strong>다음글</strong>
									<c:if test="${detail1.noticeIdx ==null}">	// 
											<a>다음글 없습니다.</a>
									</c:if>
									<c:if test="${detail1.noticeIdx !=  null}">
													<a href="javascript:moveDetailViewPaging('${detail1.noticeIdx}','${pageNum+1}');" id="snominus"><c:out value="${detail1.noticeTitle}"/></a>
