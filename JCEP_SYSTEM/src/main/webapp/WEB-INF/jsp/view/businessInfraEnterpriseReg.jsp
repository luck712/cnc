										...
										<tr>
											...
											<td>
												<label class="input" style="width:100%;">
													<input type="text" id="entprPerformNm" name="entprPerformNm" class="input-sm" value="<c:out value="${resultList[0].entprPerformNm}"/>" style="width:100%;" maxlength="100">
												</label>
											</td>
										</tr>
									</tbody>
	...
	var resourFaciChecked2 = new Array();
		...
		function goResourFaciSearch(flag){
			...
			if(flag === "Y"){
			 ...
			 $.ajax({
					type : 'post',
					url:'/db/business/bussInfraEntprResourFaciSearchList.do',
		...
		function selectResourFaci(){
			...
			var param = {
			 ...
			 $.ajax({
					type : 'post',
					url:'/db/business/bussInfraEntprResourFaciSelectList.do',
					data: param,
					dataType: 'json',
					success : function(data) {
						var html = "";
		...
		function fn_save(){
			if(checkValue()){
				...
				var entprPerformNm =  $('#entprPerformNm').val(); // 제목
				...
				$("#resultTbody tr").each(function(i){
					var mapData = {
							...
							"entprPerformNm"		: entprPerformNm,
							"resourFaciId"			: $(this).attr('value'),
							"memberId"				: $(this).attr('value2'),
							"appliNo"					: $(this).attr('value3'),
							"mainBussCont"		: $(this).find("td:eq(2)").find("input").val(),
							"moveInDt"				: $(this).find("td:eq(3)").find("input").val(),
					}
					listData.push(mapData);
				});
				 $.ajax({
						type : 'post',
						url:'/db/business/bussInfraEntprResourFaciSave.do',
