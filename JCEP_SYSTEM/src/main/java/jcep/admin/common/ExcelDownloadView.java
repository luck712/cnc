...
public class ExcelDownloadView extends AbstractView {
    ...
    protected final void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        buildExcelDocument(model, workbook, request, response); //여기로 들어가 
        ...
        // Don't close the stream - we didn't open it, so let the container handle it.//뭐라는거야 이새기야 ;
    ...
    protected void buildExcelDocument(Map model, XSSFWorkbook wb, HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        List<HashMap<String,String>> dataList = (List<HashMap<String, String>>) dataMap.get("list"); // 데이터가 담긴 리스트 
        ...
        if(dataList.size() > 0){ // 저장된 데이터가 있을때
            ...
            for (int i = 0; i<dataList.size(); i++) {
        		int j =0;
            	for(Entry<String, String> data : dataList.get(i).entrySet()) {
            		cell = getCell(sheet, 1 + i, j);
                    setText(cell, data.getValue()); 
                   cell.setCellStyle(cellStyle2);
                   j++;
            	}
            }
